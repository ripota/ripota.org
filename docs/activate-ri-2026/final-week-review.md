...
Reviewed September 5, 2026 against main `2a8276b5`, current production pages, public schedule data, open GitHub issues, browser tests, and outside reporting. This is an assessment and implementation plan. The proposed site changes have not been applied.
...
Three independent reviews covered hunters, activators, and external reporting. Public pages were inspected on desktop and at phone widths. The full existing `mise run e2e:activate-ri` suite passed on this revision, including its local build; `test-results/.last-run.json` reported `passed` with no failed tests. A separate browser exercise using synthetic local D1 data then reproduced a cancellation defect that the passing suite does not cover. No real registrations, messages, accounts, or production configuration were changed.
...
| First-time or one-stop activator | Volunteer without understanding the whole application | Signup, organizer review, email access, official guide | “Add one park or a multi-park route.” Require an intentional date choice; make email sign-in easy to see. |
...
| Checklist privacy and manual controls | Visible: “Your CSV stays on this device. Your checklist is saved in this browser only.” Preserve the full disclosure under “Privacy details.” Add “Check a park after you work it. This changes your checklist; official POTA credit comes from activator logs.” Explain that reimport preserves manual choices. |
...
| Activator signup / sign-in | “Add your activation plan” and “Sign in to change your plan.” Keep passkeys and email links; show both choices without exposing authentication terminology in the lead. Describe link expiry when sending/using the link. Do not alter authentication policy this week. |
...
Relevant files: `src/pages/activate-ri-2026/`, `src/components/activate-ri/`, `src/components/auth/SignInPanel.astro`, `src/data/site.ts`, and `src/components/SiteHeader.astro`. This is the second work package: roughly half a day for the focused copy/navigation pass, followed by a rendered review.

**Make schedule and listing pages work better on a phone**

The production schedule's first row starts about 1,156 pixels below the top at 390 × 844, after seven stacked filters. It does not overflow horizontally; the problem is how much setup precedes useful information. The hunter upload field similarly sits below the first screen, and the full import instructions remain above saved results on repeat visits.
...
3. Keep park reference/name, callsign, date/time/zone, bands/modes, and changed/cancelled status readable together. Reuse the existing callsign popover and public notes; make important changed-plan notes easy to discover.
...
| September 8 | 60–90 minute rehearsal with one hunter, one activator/rover, and an organizer; include someone unfamiliar with the site | Record completed task, time, confusion, and observed result rather than “looks good” |
...
| September 10–13 | Use existing tools; make corrections as plans change; protect time for operating/support | Current schedule, clearly dated live/progress data, resolved coverage problems |
...
- **Activator:** use a supported real phone's email app/browser and passkey flow; create a synthetic one-stop and club/rove plan in the local test environment; approve, edit, remove, cancel, reload, and re-save. Confirm public results after each operation. Try an expired email link and the existing fallback. The virtual authenticator does not prove iCloud/Google password manager or in-app email-browser behavior.
...
At review time GitHub showed no open PRs, but there were several local workspaces and an open park-report backlog. A workspace name alone does not show active work. Check the current branch before starting each package.
