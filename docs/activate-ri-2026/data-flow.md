...
If that API is unavailable in a production build, public consumers show an
unavailable state rather than treating a static snapshot as current plans.
Maps retain neutral park locations and do not infer coverage gaps. Astro dev
alone can use a local `stops.json` export with a valid `generatedAt` timestamp;
the checked-in undated empty placeholder is never a usable schedule snapshot.
